...
public class RenderBatch implements Comparable<RenderBatch>{
	...
	private final int ENTITY_ID_OFFSET = TEX_COORDS_OFFSET + ENTITY_ID_SIZE * Float.BYTES;
	...
	private void loadVertexProperties(int index) {
		...
		// Find the offset within array (4 vertivces per sprite)
		int offset = index * 4 * VERTEX_SIZE;
		// Float float float, flaot, float, float
		...
		for (int i = 0; i < 4; i++) {
			...
			vertices[offset + 3] = color.y; // B
			vertices[offset + 4] = color.z; // G
			vertices[offset + 5] = color.w; // A

			// Load position
			vertices[offset + 6] = texCoords[i].x; // G
